use crate::{
	...
	rest::{problem, problem::ProblemResponse},
};
use rocket::{
	http::Status,
...
pub async fn register_github_user(
	...
	let access_token =
		github_identity_provider.new_access_token(registration.authorization_code).await;
	...
	let result = badge_registry_client.register_user(registration.account_address, user_id).await;
